...
# The photo is wider than tall. It meets the square halfway: its left end
# is cut by half the difference - the shotgun reaches the right edge, so
# the right end stays - and bands filled from its own background make up
# the other half, above and below. Painted on rounded corners, 256 px: the
# Marketplace wants at least 128 and never an SVG. Run it again after
# changing the photo:
#
...
# the square's side, in photo pixels: halfway between its height and width
$scale = $n / (($img.Width + $img.Height) / 2)
$w = [int][Math]::Round($img.Width * $scale)
$h = [int][Math]::Round($img.Height * $scale)
$dest = New-Object System.Drawing.Rectangle ($n - $w), ([int](($n - $h) / 2)), $w, $h
...
function Add-Band([int]$Y, [int]$H, $From, $To) {
...
$g.Flush()
if ($dest.Y -gt 0) {
    Add-Band 0 $dest.Y $fill (Get-RowColor $dest.Y)
    Add-Band $dest.Bottom ($n - $dest.Bottom) (Get-RowColor ($dest.Bottom - 1)) $fill
}
$g.Dispose()
$img.Dispose()
